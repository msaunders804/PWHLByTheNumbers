#!/usr/bin/env python3
"""
PWHL Skater Points Leaders Visualization
Creates professional graphics for top point scorers
...
def fetch_skater_leaders(season_id=8, limit=10):
    """
    Fetch top skater point leaders from official PWHL API
    ...
    try:
        print(f"Fetching skater leaders from PWHL API (Season {season_id})...")
        response = requests.get(API_BASE_URL, params=params, timeout=10)
        response.raise_for_status()
        data = response.json()

        players = data.get('SiteKit', {}).get('Statviewtype', [])
        print(f"Retrieved {len(players)} skaters")
        return players

    except Exception as e:
        print(f"Error fetching skater leaders: {e}")
        return []
...
def create_skater_leaders_viz(season_id=8, limit=10, output_file=None):
    """
    Create skater points leaders visualization
    ...
    """
    # Fetch data
    players = fetch_skater_leaders(season_id, limit)
    ...
    title_text = "PWHL POINTS LEADERS"
    ...
    if not output_file:
        ...
        output_file = os.path.join(output_dir, f'pwhl_skater_leaders_{timestamp}.png')
...
def main():
    """Main function"""
    print("=" * 70)
    print("PWHL SKATER POINTS LEADERS VISUALIZATION")
    print("=" * 70)

    import argparse
    parser = argparse.ArgumentParser(description='Create PWHL skater leaders visualization')
    ...
    print(f"\nCreating skater leaders visualization for season {args.season}...")
    output_file = create_skater_leaders_viz(args.season, args.limit, args.output)
