...
def _pwhl_logo_uri():
    for ext in ("png", "svg", "jpg", "webp"):
        p = Path(__file__).parent / "assets" / "logos" / f"PWHL_logo.{ext}"
        if p.exists():
...
def main():
    ...
    parser.add_argument("--skater-photo",   type=str, default=None,
                        help="Path to skater photo for slide 1")
    ...
    # Manual --skater-photo overrides the auto-picked player from DB
    if args.skater_photo:
        from pathlib import Path as _P
        p = _P(args.skater_photo)
        data["skater_photo"] = p.resolve().as_uri() if p.exists() else args.skater_photo
    if args.skater_name:  data["skater_name"] = args.skater_name
    if args.skater_team:  data["skater_team"] = args.skater_team
    # Log who was auto-picked for slide 1
